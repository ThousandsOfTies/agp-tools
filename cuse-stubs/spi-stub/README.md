...
（`AGP_HW_SIM_SOCK` または `AGP_RUNTIME_DIR/hw_sim.sock`）から取得するので、`agp sim ui rfid tap <UID>` でタップを
注入できます。
...
責務をアプリから AgentCockpit runtime 側へ閉じ込めるのが狙いです。
...
`-f` は foreground 実行。バックグラウンド常駐は `agp sim start` に組み込みます
...
1. `agp sim start` で `cuse_spi` が常駐し `/dev/spidev0.0` が見える
2. `~/sensor_demo`（LD_PRELOAD なし）が VersionReg=0x92 を読めて初期化成功
3. `agp sim rfid tap 04:AB:CD:EF:01:23` 後に `sensor_demo` が UID を表示する
4. `agp sim rfid remove` でカード無し挙動に戻る
